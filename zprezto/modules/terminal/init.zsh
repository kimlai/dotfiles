...
function _terminal-set-titles-with-command {
  ...
  if [[ "${2[(w)1]}" == (fg|%*)(\;|) ]]; then
    ...
    set-window-title "$3" # simply show the whole command with arguments (third argument of the preexec hook)
  fi
}
...
function _terminal-set-titles-with-path {
  ...
  # Replicate tmux's default behaviour
  # don't show the current path in the window-title (which is also Kitty's tab title)
  # the path is displayed only when nothing is running in the foreground, ie it's a shell prompt
  set-window-title "zsh"
}
